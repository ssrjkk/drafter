...
- **Живое демо:** https://ssrjkk.github.io/drafter/
...
- **Выбор провайдера и модели** — переключение между 9 AI-провайдерами и их моделями на лету
- **Горячие клавиши** — Ctrl+/ для справки по горячим клавишам, Ctrl+K для командной палитры
...
src/
  components/
    ...
    layout/            # MainLayout
    modals/            # Модалки API-ключа, мастер-пароля, онбординга
...
Все опциональные. Приложение полностью настраивается через модалку настроек (API-ключи, провайдер, модель).

```env
# .env (опционально)
VITE_BASE=/drafter/          # базовый путь для собранного приложения (по умолчанию: /)
